...
组件实例在使用仓库中的bannerList，组件的这个属性bannerList一定是发生过变化，watch可以监听到。但是因为只要数据变化就会监听所以不够，引入nextTicket
...
在循环结束后收到数据之后，也就是渲染结束后延迟实现
注意：组件实例的$nextTick方法，在工作当中经常使用，保页面的结构一定是有的，经常结合第三方插件使用，获取更新后的DOM节点
...
仓库中的state数据格式取决于服务器返回数组
3.1:Floor组件获取mock数据，发请求的action书写在哪里?
因为mockd的数据是两个轮播图数据都有，所以要再父组件书写，也就是home组件
...
4)carousel全局组件
如果项目当中出现类似的功能，且重复利用，封装为全局组件----【不封装也可以】,封装的话样式起码要一样的
