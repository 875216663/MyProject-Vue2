      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt" v-model="skuNum" @change="changeSkuNumber"/>
                <a href="javascript:" class="plus"  @click="skuNum++">+</a>
                <a href="javascript:" class="mins"  @click="skuNum > 1 ? skuNum-- : (skuNum = 1)">-</a>
              </div>
              <div class="add">
                <!-- 以前路由跳转，从a跳到b，在这里加入购物车，跳转前，先发请求，阀产品信息通知路由器 -->
...
export default {
  ...
  computed: {
    ...
    //如果服务器还没有传回来数据的时候，最少传一个空数组
    skuImageList() {
      return this.skuInfo.skuImageList || [];
    },
  },
  methods: {
    //就是把售卖属性遍历一遍，把所有的ischeck都变成0，然后点击的那个ischeck变成1
    ...
    //表单元素修改产品个数
    changeSkuNumber() {
      //用户输入进来的文本非法的时候，比如不是数字或者小于1
      if (isNaN(this.skuNum) || this.skuNum < 1) {
        this.skuNum = 1;
      } else {
        //parseInt 只会保留字符串中最前面的整数部分，并忽略小数部分
        this.skuNum = parseInt(this.skuNum);
      }
    },
    
    //加入购物车的回调函数，为什么使用async，因为要等待服务器返回数据
    async addShopCar() {
      ...
      //服务器没有返回数据
      //一些简单的数据skuNum，通过query形式给路由组件传递过去,这个没有用因为地址太复杂了
      //产品信息的数据【比较复杂:skuInfo】,通过会话存储（不持久化,会话结束数据在消失）
      //本地存储|会话存储，一般存储的是字符串，不能存储对象，所以需要序列化
      //4：在路由跳转的时候还需要将产品的信息带给下一级的路由组件
      try {
